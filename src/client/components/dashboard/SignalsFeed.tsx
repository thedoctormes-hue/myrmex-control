...
export function SignalsFeed({ changelog }: { changelog: ChangelogEntry[] }) {
  ...
  return (
    <div className="bg-card border border-border rounded-lg p-4 md:col-span-2">
      ...
      {recent.length === 0 ? (
        <p className="text-xs text-muted-foreground-foreground">Нет событий</p>
      ) : (
        <div className="space-y-2">
          {recent.map(entry => (
            <div key={entry.id} className="flex items-start gap-2 text-xs">
              <span className="text-muted-foreground-foreground whitespace-nowrap">
                {new Date(entry.timestamp).toLocaleTimeString('ru')}
              </span>
              <span className="text-primary">{entry.source}</span>
              <span className="text-muted-foreground-foreground">{entry.action}</span>
